...
Jet_pT: jet pT
*/
double GetUE_JESCorrection(char jetcollection, bool doUE, float Jet_pT, bool isemb){

	double correction = 1.0;
	TF1 *CorrectionFunction = new TF1("CorrectionFunction","([0]*x)/(x+[1])",30.0,800.0);

	if(jetcollection.c_str() == "akCs4PFJetAnalyzer" && !doUE && isemb){
		CorrectionFunction->SetParameters(1.00168e+00, -2.27352e+00);
        correction = CorrectionFunction->Eval(Jet_pT);
	}

	if(jetcollection.c_str() == "ak4PFJetAnalyzer" && doUE && isemb){
